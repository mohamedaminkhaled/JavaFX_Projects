...
public class SalesReportController {
    ...
    void setReport(LocalDate dateFrom, LocalDate dateTo) throws SQLException, IOException, ClassNotFoundException {
    	...
    	String selectBill = "SELECT items.name, sum(sales.quantity) AS sum_quantity, "
    					  + "sum(sales.price) AS sum_price FROM items INNER JOIN sales "
    					  + "on items.id = sales.id WHERE `time` >= '"+ dateFrom +"' AND `time` <= '"+ dateTo +"' GROUP BY items.name";
		...
		lblNewTotal.setText(strNewTotal+" EP");
