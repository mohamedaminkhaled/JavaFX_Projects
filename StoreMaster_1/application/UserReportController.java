...
public class UserReportController {
    ...
    void setReport(LocalDate dateFrom, LocalDate dateTo) throws SQLException, IOException, ClassNotFoundException {
    	...
    	String selectBill = "SELECT `loan`, `time` FROM `user_loans` "
		...
		do {
			...
			//quantity
			String strLoan = rs.getString("loan");
			String strTime = rs.getString("time");
			
			double price = Double.parseDouble(strLoan);
			totalCoast += price;
			
			//add new item to report
			Label lblLoan = new Label(strLoan);
			lblLoan.setPrefWidth(115);
			lblLoan.setStyle("-fx-font-size: 12; -fx-padding:0 0 0 10;");
			
			Label lblTime = new Label(strTime);
			lblTime.setPrefWidth(175);
			lblTime.setStyle("-fx-font-size: 12; -fx-padding:0 0 0 5;");
						
			HBox newItem = new HBox(5);
			newItem.setPrefWidth(317);
			newItem.setPrefHeight(20);
			
			newItem.getChildren().addAll(lblLoan, lblTime);
